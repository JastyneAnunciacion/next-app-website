...
const LeaderboardRankTemplate = ({
    ...
    return (
        <>
            {/* Web Ver */}
            <div className={`${isEven && 'bg-[#130C28]'} ${!isLast && 'mb-[0.49%]'} w-full aspect-[103/4] rounded-[10px] hidden lg:grid grid-cols-3 items-center px-[0.97%]`}>
            ...
            {/* Mobile Ver */}
            <div className={`${isEven && 'bg-[#403269]'} w-full grid lg:hidden grid-cols-2 items-center px-[5vw] py-[1.56vw]`}>
                <div className='flex items-center gap-[2.08vw]'>
                    <div className='bg-[#332559] w-[4.16vw] aspect-square text-[2.5vw] flex items-center justify-center rounded-[5px]'>{rank}</div>
                    <ImageResponsive src={playerProfilePictureSrc} alt='Player Profile Image' width='5.21vw' />
                    <p>{playerName}</p>
                </div>
                <div className='flex justify-end'>
                    <p className='text-[#02CC00] mr-[0.89%]'>{'$' + tempTempPrize}</p>
