...
const SideBarListItem = ({ isSideBarOpen, itemName, pageHref }: SideBarListItemProps) => {
    ...
    return (
        <li>
            {isSideBarOpen ? (
                <Link href={pageHref} className='w-full h-11 flex items-center justify-center gap-2'>
                ...
                <Link href={pageHref} className='relative w-9 h-9 flex items-center justify-center hover:bg-blue-400 hover:rounded-full group'>
                    <div className='h-7 w-7 bg-gray-300 rounded-full' />
                    {showItemName && (
